...
// UpdateDepositTxID sets the reserve_deposit_tx_id on a mint request inside a transaction.
func (r *MintRepository) UpdateDepositTxID(ctx context.Context, tx *sql.Tx, requestID string, depositTxID string) error {
	_, err := tx.ExecContext(ctx,
		`UPDATE mint_requests SET reserve_deposit_tx_id = $1, updated_at = $2 WHERE request_id = $3`,
		depositTxID, time.Now().UTC(), requestID,
	)
	if err != nil {
		return fmt.Errorf("update deposit tx id for %s: %w", requestID, err)
	}
	return nil
}
...
// CheckDailyLimit computes the total minted amount for a wallet on a given date
// and compares against the policy limit. Returns an error if the wallet's daily
// total plus the new amount would exceed the daily limit, or if the wallet's
// cumulative total would exceed the per-wallet limit.
//
// Day total is derived by aggregating mint_requests where status is NOT
// 'cancelled' or 'failed' for the given wallet and date.
func (r *MintRepository) CheckDailyLimit(ctx context.Context, wallet string, newAmountMinor int64, date string, policy *model.MintPolicy) error {
	// Aggregate daily mint amount for this wallet on this date.
	// Count only requests that are not cancelled/failed (exclude terminal failures).
	...
	err := r.db.QueryRowContext(ctx, dailyQuery, wallet, date).Scan(&dailyTotal)
	if err != nil {
	...
	// Aggregate lifetime mint amount for this wallet (all time, not just today).
	...
	err = r.db.QueryRowContext(ctx, lifetimeQuery, wallet).Scan(&lifetimeTotal)
	if err != nil {
