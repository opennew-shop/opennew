...
const STATE_FILE = process.env.ANCF_STATE_FILE
  ? path.resolve(process.env.ANCF_STATE_FILE)
  : path.join(__dirname, '.mock-state.json');
const MAX_QUOTE_QUANTITY = parseInt(process.env.ANCF_MAX_QUOTE_QUANTITY || '10', 10);
const ALLOWED_ORIGINS = (process.env.ANCF_ALLOWED_ORIGINS || 'https://opennew.shop,http://127.0.0.1:3000,http://127.0.0.1:8080')
  .split(',')
  .map(s => s.trim())
  .filter(Boolean);
let _reqOrigin = '';
...
// ---- File Persistence ----
let persistedState = {};
try { persistedState = JSON.parse(fs.readFileSync(STATE_FILE, 'utf-8')); console.log('[Persistence] Loaded state from ' + STATE_FILE); }
...
const SKUS = [
  {
    ...
    media: { thumbnail: 'https://opennew.shop/assets/products/h100.svg' },
  ...
  },
  {
    ...
    media: { thumbnail: 'https://opennew.shop/assets/products/a100.svg' },
    embedding: 'pending' // pgvector embedding generated by catalog service (1536-dim)
  },
  {
    ...
    media: { thumbnail: 'https://opennew.shop/assets/products/l40s.svg' },
    embedding: 'pending' // pgvector embedding generated by catalog service (1536-dim)
  }
];
...
const AGENT_QUOTE_REQUESTS = {}; // agent_id → quote request inbox

// ---- In-memory store ----
const quotes = {};
const intents = {};
const idempotencyKeys = persistedState.idempotencyKeys || {};
...
function allowedOrigin() {
  if (!_reqOrigin) return ALLOWED_ORIGINS[0] || 'https://opennew.shop';
  return ALLOWED_ORIGINS.includes(_reqOrigin) ? _reqOrigin : ALLOWED_ORIGINS[0] || 'https://opennew.shop';
}
...
function badRequest(res, message, err) {
  if (err) console.warn('[Mock API] ' + message + ':', err.message);
  return jsonResponse(res, 400, { code: 400, message });
}
function isUnsignedMinor(value) {
  return typeof value === 'string' && /^\d+$/.test(value);
}
function isPositiveMinor(value) {
  return isUnsignedMinor(value) && BigInt(value) > BigInt(0);
}
...
function getSKU(skuID) {
  return SKU_STORE[skuID] || SKUS.find(s => s.sku_id === skuID);
}
function sellerAgentIDForSKU(sku) {
  return sku && sku.agent_id ? sku.agent_id : 'seed';
}
function stockOf(sku) {
  if (!sku) return 0;
  if (typeof sku.stock === 'number') return sku.stock;
  if (typeof sku.stock_hint === 'number') return sku.stock_hint;
  return 0;
}
function searchItemOf(sku) {
  const price = sku.price || {
    currency: sku.currency || 'AGP',
    amount_minor: String(sku.price_amount_minor || 0),
    scale: sku.price_scale || 6
  };
  const item = {
    sku_id: sku.sku_id,
    title: sku.title,
    ...
    price: {
      currency: price.currency || 'AGP',
      amount_minor: String(price.amount_minor || 0),
      scale: Number(price.scale || 6)
    },
    stock_hint: stockOf(sku),
    specs: sku.specs || {},
    media: sku.media || {},
    category: sku.category || (sku.specs && sku.specs.GPU ? 'GPU' : 'API')
  };
  return item;
}
function decrementStock(skuID, qty) {
  const sku = SKU_STORE[skuID];
  if (!sku) return false;
  const current = stockOf(sku);
  if (current < qty) return false;
  sku.stock = current - qty;
  sku.stock_hint = sku.stock;
...
function productAssetSVG(name, label, accent) {
...
const manifest = {
  ...
  agent_policy: {
    ...
    allowed_component_hosts: ['opennew.shop', 'cdn.yourshop.com']
  },
  payment_rails: [
    {
      rail: 'agp_ledger',
...
const server = http.createServer((req, res) => {
  ...
  _reqOrigin = String(req.headers.origin || '');
  ...
  // ---- Static demo product media ----
  if (method === 'GET' && path.startsWith('/assets/products/')) {
  ...
  if (method === 'POST' && path === '/api/v1/auth/register-agent') {
    ...
    req.on('end', () => {
      try {
        ...
        return badRequest(res, 'Invalid register-agent request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/auth/bind-wallet') {
    ...
    req.on('end', () => {
      try {
        ...
        return badRequest(res, 'Invalid bind-wallet request', e);
      }
    });
    return;
  }
  ...
  // ---- GET /api/v1/agent/quote-requests ----
  if (method === 'GET' && path === '/api/v1/agent/quote-requests') {
    const auth = verifyAgentToken(req);
    if (!auth) {
      return jsonResponse(res, 401, { code: 401, message: 'Valid X-ANCF-Agent-Token header is required' });
    }

    const limitRaw = parseInt(parsedUrl.searchParams.get('limit') || '50', 10);
    const limit = Number.isSafeInteger(limitRaw) ? Math.min(Math.max(limitRaw, 1), 100) : 50;
    const status = parsedUrl.searchParams.get('status');
    let requests = AGENT_QUOTE_REQUESTS[auth.agent_id] || [];
    if (status) requests = requests.filter(qr => qr.status === status);
    return jsonResponse(res, 200, {
      agent_id: auth.agent_id,
      count: requests.length,
      quote_requests: requests.slice(0, limit)
    });
  }
  ...
  if (method === 'GET' && path === '/api/v1/cli/search') {
    ...
    const allProducts = [...SKUS, ...Object.values(SKU_STORE).filter(s => !SKUS.find(seed => seed.sku_id === s.sku_id))].map(searchItemOf);
  ...
  if (method === 'GET' && path === '/api/v1/cli/rag-search') {
    ...
    const allRAG = [...SKUS, ...Object.values(SKU_STORE).filter(s => !SKUS.find(seed => seed.sku_id === s.sku_id))].map(searchItemOf);
    let results = allRAG;
    if (q) {
    ...
    req.on('end', () => {
      try {
        ...
        for (const line of data.lines) {
          ...
          if (!qty) return jsonResponse(res, 400, { code: 400, message: 'Invalid quantity' });
          if (qty > MAX_QUOTE_QUANTITY) return jsonResponse(res, 400, { code: 400, message: `Quantity exceeds per-order limit: ${MAX_QUOTE_QUANTITY}` });

          const sku = getSKU(line.sku_id);
          if (!sku) return jsonResponse(res, 404, { code: 404, message: `SKU not found: ${line.sku_id}` });
          if (stockOf(sku) < qty) return jsonResponse(res, 409, { code: 409, message: `Insufficient stock for SKU: ${line.sku_id}` });

          const price = BigInt(sku.price?.amount_minor || sku.price_amount_minor || '0');
          const qtyBN = BigInt(qty);
          const lt = price * qtyBN;
          const sellerAgentId = sellerAgentIDForSKU(sku);
          const quoteLine = {
            sku_id: line.sku_id,
            quantity: qty,
          ...
          if (!sellerLines[sellerAgentId]) sellerLines[sellerAgentId] = [];
          sellerLines[sellerAgentId].push(quoteLine);
        }
        const total = lines.reduce((sum, l) => sum + BigInt(l.line_total_minor), BigInt(0));
        const quoteId = generateId('quote_');
        const sellerAgentIds = Object.keys(sellerLines);
        const quote = {
          quote_id: quoteId,
          currency: 'AGP',
          ...
          lines: lines,
          seller_agent_ids: sellerAgentIds
        };
        quotes[quoteId] = {
          ...quote,
          consumed: false,
          ...
          _seller_agent_id: sellerAgentIds.length === 1 ? sellerAgentIds[0] : undefined,
          _seller_agent_ids: sellerAgentIds
        };

        const now = new Date().toISOString();
        for (const sellerAgentId of sellerAgentIds) {
          if (!AGENT_QUOTE_REQUESTS[sellerAgentId]) AGENT_QUOTE_REQUESTS[sellerAgentId] = [];
          const sellerTotal = sellerLines[sellerAgentId].reduce((sum, l) => sum + BigInt(l.line_total_minor), BigInt(0));
          AGENT_QUOTE_REQUESTS[sellerAgentId].unshift({
            request_id: generateId('qr_'),
            quote_id: quoteId,
            status: 'created',
            ...
            currency: 'AGP',
            scale: 6,
            total_minor: sellerTotal.toString(),
            lines: sellerLines[sellerAgentId],
            created_at: now,
            expires_at: quote.expires_at
          });
        }

        console.log(`[Mock API] Quote created: ${quoteId}, total=${total}AGP, sellers=${sellerAgentIds.join(',') || 'none'}`);
        return jsonResponse(res, 200, quote);
      } catch (e) {
        console.warn('[Mock API] Invalid quote request:', e.message);
        return badRequest(res, 'Invalid quote request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/cli/checkout/prepare') {
    ...
    req.on('end', () => {
      try {
        ...
        console.warn('[Mock API] Invalid checkout prepare request:', e.message);
        return badRequest(res, 'Invalid checkout prepare request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/cli/checkout/commit') {
    ...
    req.on('end', () => {
      try {
        ...
        const cached = idempotencyKeys[idempotencyKey];
        if (cached) {
          if (cached.body_hash !== bodyHash) {
            return jsonResponse(res, 409, { code: 409, message: 'Idempotency-Key was used with a different request body' });
          }
          return jsonResponse(res, cached.status_code || 200, cached.response_body);
        }
        ...
        if (!sigCheck.ok) {
          ...
          return jsonResponse(res, 401, { code: 401, message: 'wallet signature verification failed' });
        }

        for (const line of quote.lines || []) {
          const qty = parsePositiveInteger(line.quantity);
          const sku = getSKU(line.sku_id);
          if (!sku || !qty || stockOf(sku) < qty) {
            return jsonResponse(res, 409, { code: 409, message: `Insufficient stock for SKU: ${line.sku_id}` });
          }
        }
        for (const line of quote.lines || []) {
          decrementStock(line.sku_id, parsePositiveInteger(line.quantity));
        }
        ...
        console.warn('[Mock API] Invalid checkout commit request:', e.message);
        return badRequest(res, 'Invalid checkout commit request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/wallet/deposit-intents') {
    ...
    req.on('end', () => {
      try {
        ...
        return badRequest(res, 'Invalid deposit intent request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/wallet/deposit-confirm') {
    ...
    req.on('end', () => {
      try {
        ...
        const amountStr = String(data.amount_minor || '');
        if (!isPositiveMinor(amountStr)) {
          return jsonResponse(res, 400, { code: 400, message: 'amount_minor must be a positive integer string' });
        }
        const mintReq = {
          request_id: mintId,
          deposit_intent_id: intentId,
        ...
        return badRequest(res, 'Invalid deposit confirm request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/wallet/redeem') {
    ...
    req.on('end', () => {
      try {
        ...
        const amountMinor = String(data.amount_minor);
        ...
        console.log(`[Mock] Redemption created: ${redeemId}, wallet=${data.wallet}, amount=${amountMinor}`);
        return jsonResponse(res, 201, redeem);
      } catch(e) {
        return badRequest(res, 'Invalid redeem request', e);
      }
    });
    return;
  }
    ...
    if (payoutMatch) {
      ...
      req.on('end', () => {
        try {
          ...
          return badRequest(res, 'Invalid redeem payout request', e);
        }
      });
      return;
    }
  ...
  if (method === 'POST' && path === '/api/v1/payments/create-link') {
    ...
    req.on('end', () => {
      try {
        ...
        const amountRaw = String(data.amount);
        if (!isPositiveMinor(amountRaw)) {
          return jsonResponse(res, 400, { code: 400, message: 'amount must be a positive integer string' });
        }
        ...
        return badRequest(res, 'Invalid payment link request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/catalog/products') {
    ...
    req.on('end', () => {
      try {
        ...
        const priceMinor = parseUnsignedInteger(data.amount_minor);
        if (priceMinor === null) {
        ...
        const sku = {
          ...
          specs: data.specs || {},
          media: data.media || {},
          category: data.category || (data.specs && data.specs.GPU ? 'GPU' : 'API'),
          status: 'active',
      ...
      } catch (e) { return badRequest(res, 'Invalid request', e); }
    });
    return;
  }
      ...
      req.on('end', () => {
        try {
          ...
          if (data.amount_minor !== undefined) {
            const pm = parseUnsignedInteger(data.amount_minor);
            if (pm === null) return jsonResponse(res, 400, { code: 400, message: 'Invalid amount_minor' });
          ...
          if (data.category !== undefined) sku.category = data.category;
  ...
  if (method === 'POST' && path === '/api/v1/dispute/file') {
    ...
    req.on('end', () => {
      try {
        ...
        return badRequest(res, 'Invalid dispute file request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/dispute/vote') {
    ...
    req.on('end', () => {
      try {
        ...
        return badRequest(res, 'Invalid vote request', e);
      }
    });
    return;
  }
  ...
  if (method === 'POST' && path === '/api/v1/dispute/join-committee') {
    ...
    req.on('end', () => {
      try {
        ...
        return badRequest(res, 'Invalid join-committee request', e);
      }
    });
    return;
  }
...
function getStateFn() { return { SKU_STORE, quotes, intents, AGENT_TOKENS, AGENT_PRODUCTS, AGENT_WALLETS, AGENT_QUOTE_REQUESTS, DEPOSIT_INTENTS: depositIntents, MINT_REQUESTS: mintRequests, REDEMPTION_REQUESTS: redemptionRequests, ESCROW_ACCOUNTS, transactionRegistry, DISPUTES, AGP_BALANCES, DISPUTE_ESCROW, paymentLinks, idempotencyKeys }; }
...
if (persistedState.AGENT_QUOTE_REQUESTS) Object.assign(AGENT_QUOTE_REQUESTS, persistedState.AGENT_QUOTE_REQUESTS);
...
if (persistedState.idempotencyKeys) Object.assign(idempotencyKeys, persistedState.idempotencyKeys);
