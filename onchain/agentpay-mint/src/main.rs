//! ANCF AGP Token Mint — Solana Token-2022
//!
//! 按照 Solana 基金会官方文档实现:
...
#[derive(Parser, Debug)]
#[command(name = "AGP-mint")]
#[command(about = "Deploy and mint ANCF AGP Token-2022 on Solana", version = "1.0")]
struct Cli {
    ...
    /// AGP token decimals (default: 6, matching USDC)
    #[arg(long, default_value_t = 6)]
    decimals: u8,

    /// Initial mint amount in token native units (e.g. 1000000 = 1.0 AGP with 6 decimals)
    #[arg(long, default_value_t = 1_000_000_000_000)] // 1M AGP
    mint_amount: u64,
...
fn main() -> Result<()> {
    ...
    println!("╔══════════════════════════════════════════════╗");
    println!("║  ANCF AGP Token-2022 Mint                  ║");
    println!("╠══════════════════════════════════════════════╣");
    ...
    // ── Step 5: Mint tokens ──
    println!("[5/6] Minting AGP tokens...");
    println!(
        "      Amount: {} ({} AGP)",
        cli.mint_amount,
        format_amount(cli.mint_amount, cli.decimals)
    ...
    if !cli.dry_run {
        ...
        println!(
            "      ATA balance: {} {}",
            token_balance.ui_amount_string, "AGP"
        );

        let mint_supply = rpc.get_token_supply(&mint_pubkey)?;
        println!(
            "      Total supply: {} {}",
            mint_supply.ui_amount_string, "AGP"
        );
    } else {
    ...
    if !cli.dry_run {
        println!("╚══════════════════════════════════════════════╝");
        println!();
        println!("✅ AGP Token-2022 mint deployed successfully!");
        println!();
        println!("Next steps:");
        println!(
...
fn format_amount(amount: u64, decimals: u8) -> String {
    ...
    if decimals == 0 {
        return format!("{} AGP", amount);
    }
    format!("{:.6} AGP", amount as f64 / divisor)
}
...
mod tests {
    ...
    fn test_format_amount() {
        assert_eq!(format_amount(1_000_000, 6), "1.000000 AGP");
        assert_eq!(format_amount(2_450_000, 6), "2.450000 AGP");
        assert_eq!(format_amount(0, 6), "0.000000 AGP");
        assert_eq!(format_amount(1_000_000_000_000, 6), "1000000.000000 AGP");
    }

    #[test]
    fn test_format_amount_zero_decimals() {
        assert_eq!(format_amount(100, 0), "100 AGP");
    }
